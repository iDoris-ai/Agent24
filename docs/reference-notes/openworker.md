...
> 所有 `path:line` 相对 `vendor/reference/openworker/`（涉及本仓库代码时给出仓库内完整路径）。
>
> **2026-07-25 经 codex 对抗式评审修订**：§2/§5/§7/§11 有事实更正（初稿把「上游有」误读成「我们没有」两次），
> §12 是评审补上的四处遗漏。修订处均就地标注，没有删除原判断——错在哪里比结论更值得留档。
...
**保守默认不配 override，实际体验会很差**——接一个 filesystem MCP server，`read_file` 每次都弹审批。
（初稿写的是「等于不可用」，**过满**：D3 Guardian 已能对 gated call 做本地小模型评估并自动放行低风险，
`agent24-policy/src/lib.rs:166` + `guardian.rs:142`。但 Guardian 默认关（`agent24d/src/server.rs:79`，
刻意不默认信任模型），且是**非确定性的模型推断**，不是用户可回看可撤销的**声明式规则**——两者都要，不是二选一。）
...
> **⚠️ 但这正是它对我们不便宜的原因（codex 指出，初稿漏了）**：**Agent24 没有持久化消息线程**。
> store 只有 sessions / runs / tool_calls / approvals / schedules / audit_log
> （`agent24-store/migrations/0001_initial.sql`），agent loop 只把**成功的** exchange 追加进 session memory
> （`agent24-agent/src/lib.rs:319`）。所以「零额外格式」这个便宜我们拿不到，得先建消息表。
> 而且哈希只保证 payload 未变，**不保证世界未变**——工具可能已卸载、MCP server 已下线、目标资源已删除。
> 因此 H3 必须与 G1 的 payload 哈希 + 工具重校验 + TTL 捆绑交付，四件事缺一不可。
...
> **⚠️ 本节初稿的两条判断被 codex 评审推翻（2026-07-25 修正）——这三条策略我们基本都已经有了。**

| 策略 | 实现 | Agent24 现状（核实后） |
|---|---|---|
| **run-once-catch-up** | 循环第一趟以 `trigger="catchup"` 跑一次，把宕机期间错过的到期任务补一次（只补一次，不补 N 次） | **已有**。`agent24-scheduler/src/lib.rs:8` 的 doc contract 明写 skip-missed：「down while due fires once and jumps to its next future slot — never a replay burst」；`fire()` 从 `now` 重算 next（`:251`），`next_fire.rs:78` 同义 |
| **skip-on-overlap** | `_running_ids` 集合，上一轮没跑完就跳过这一轮 | 已有（pre-advance） |
| **spawn，不 await** | 一个 run 可能挂在 parked approval 上，**绝不能让它卡住调度循环 / 其他到期任务 / self-wake 恢复** | **已有**。scheduler 只 await `trigger()` 建 run（`lib.rs:278`），执行由 `RunManager` 另起监督 task（`agent24-agent/src/lib.rs:442`，双层 spawn 以捕获 panic） |

**教训**：我初稿凭 openfang 的对照印象写了「Agent24 未做 catch-up」，没去读自己的 scheduler doc comment。
上游对照最容易犯的错误就是**把「上游有」直接读成「我们没有」**。

仍值得留下的只有一条，且它属于 H5 而非独立任务：上游 `stop()` 的关停契约——
**关停调度器时把 spawn 出去的 run 一并 cancel，挂起的 run 不得比调度器活得久**。
另外 `extra_tick`（`scheduler.py:30,35`）这个「往 tick 里挂一个额外协程」的位置，是 self-wake 的挂载点。
...
|---|---|---|---|
| 风险模型 | 4 级 RiskClass + 分类函数 | `requires_approval: bool` | **借鉴（H1）**，但须**加法迁移**——`ToolInfo` 是公开契约（`protocol/openapi.yaml:971`），改动触发 `pnpm gen:api` + CI 零漂移门 |
| MCP 默认风险 | external（保守） | `requires_approval=true`（保守） | 一致 |
| 放宽保守默认 | 用户本地 glob override，包不得写 | 无声明式 override；有 D3 Guardian（默认关，模型推断） | **补 H2**，与 Guardian 并存：用户声明优先于模型推断 |
| 「总是允许」粒度 | `tool → 确切目标`，仅 EXTERNAL | `(session, tool)` 全参数放行，命中即 `Approved` 且**跳过 Guardian** | **收窄（H4）**，并对 external 停用宽泛选项 |
| 有人/无人 | 同一条记录，`visibility` 二选一 | 仅同步阻塞 | **借鉴**（G1 的形态答案） |
| 重启后的待审批 | durable resume，接着问 | 全部标 aborted（`agent24d/src/server.rs:388`） | **改（H3）**，但须与消息线程持久化 + payload 哈希 + 陈旧性重校验捆绑 |
| agent 自主休眠 | sleep_for / wake_on / wake_on_event | 无 | **新增（H5）** |
| 错过的定时任务 | 启动补跑一次 | ~~不补~~ → **已有 skip-missed 补跑一次** | ~~借鉴~~ **删除，我们已有** |
| 调度器与挂起 run | spawn 不 await | **已是 spawn 不 await**（`agent24-agent/src/lib.rs:442`） | ~~借鉴~~ **删除，我们已有** |
| 工具并发 | 授权串行→读并发→写串行 | 全串行 | **降级**——授权/状态转换/执行在 `dispatch()` 里耦合，非低风险改动 |
| plan mode / propose_plan | 只读门禁 + 计划审批后解锁 | 无 | **新增（H8，codex 补）** |
| 只读 explorer subagent | 独立上下文、只读工具、禁递归 | 无 | **新增（H9，codex 补）** |
| 模块安装同意 | manifest 严校验 + 默认 disabled pending consent | E3/E5 pending | **新增（H10，codex 补）** |
| 渠道可测性 | FakeSlack 协议级 hermetic fake | 无 | **新增（H11，codex 补）**——F3/F4 没有它只能手测 |
...
## 12. 初稿漏掉的四处（codex 评审补充，2026-07-25）

我初稿只重点读了权限/审批/inbox/self-wake/调度器五块，codex 翻完剩下的部分补了四条，都比原 H7 有价值：

1. **plan mode + `propose_plan`（→ H8）**：`DISCUSS` 与 `PLAN` 门禁完全相同（都只读），**区别只在意图**——
   `plan` 推着 agent 走 explore → `propose_plan` → 人批准 → 才退出只读（`permissions.py:26`、`tools/plan.py`）。
   「同一门禁、不同引导」是个干净的区分：不必为 plan 单独发明一套权限，只发明一个批准形态。
2. **只读 explorer subagent（→ H9）**：独立上下文、只读工具集、**禁递归**（`tools/subagent.py:42,128`），
   测试显式断言子 agent 拿不到 write/shell 且不能再 spawn（`tests/test_subagent.py:38`）。
   它比工具并发（H7）更直接地解决主上下文污染。对照 `openfang.md` §8 的 capability 继承校验——两者是同一件事的两面。
3. **模块/persona 安装同意（→ H10）**：manifest 严格校验（`personas/manifest.py:18,170`）→
   安装时生成 capability/risk 同意摘要且**安装路径绝不写 override**（`personas/loading.py:19`）→
   **安装后默认 disabled，pending consent**（`tests/test_persona_loading.py:44`）。
   这是 E3/E5 模块市场的前置安全边界，不是后补的 UI。
4. **协议级 Fake 渠道（→ H11）**：`FakeSlack` 是真 adapter 打一个 hermetic 的假 socket/web API
   （`coworker/testing/fake_slack/server.py`），inbound/outbound/interaction 全可自动测（`tests/test_fake_slack.py`）。
   **F3/F4 若没有 FakeWeChat/FakeNostr，渠道上的审批与 inbox 只能靠手测**——那等于没有回归保护。

另有一条小而值的：**provider 错误人话翻译**（`providers/errors.py`，按 body marker 把额度/权限类错误翻成可读文案）。
我们 `agent24-models/src/lib.rs:97` 目前还是 raw `Unavailable/Provider` 往上抛（→ H12）。
