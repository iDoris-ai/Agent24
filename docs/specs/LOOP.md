...
> loop 会话的唯一职责：按本手册推进 `TASKS.md`，直到当前目标达成或被停止。
>
> **当前目标（2026-07-27）**：按 TASKS.md「🎯 个人助理优先级」由高到低推进——
> P0 无人值守正确性（G1+H3）→ P1 渠道（微信/Nostr）→ P2 框架完成度 → P3 自主性。
> 选任务时在「pending 且依赖满足」中**优先取优先级高的**（不再是单纯序号最靠前）。
> 进入 P4（M4/M5 产品级生态）前停下与用户确认。
...
- 到达里程碑门（进入 P4 M4/M5，或任何 TASKS.md 标「需用户确认」的门）→ 停下询问，不擅自越过
- 连续 2 轮无可执行任务且无 PR 反馈 → 结束 loop 并说明等待事项
- 遇到需要用户决策的架构级冲突（spec 之间矛盾、依赖不可用、许可证问题）→ blocked 标注 + 结束本轮询问用户
- 发布动作（tag / Release / 上传）→ 永远列 checklist 交用户执行，不自行发布
