...
| 「撤 generation 早于杀进程」怎么保证 | `terminate_group` **必须消费一个 `KillPermit`**，而它只能由 `Generation::revoke` 产出 | 靠「每个调用方都记得顺序」成立的安全性质，在第一个忘了的调用方那里失效。崩溃（死掉的 leader 留下的 helper 可能还攥着回调连接）、启动超时、disable 三条路一律经过它 —— **任何豁免都是一个构造器** |
...
| 撤销时仍在途的请求 | **503 `request_abandoned`**，写日志「结果未知」；**即使模块随后答了 200**，即使连接随后断了（502） | 转发那个 200 = 为一个内核已决定停掉的运行报告成功；转发那个 502 = 把内核的 kill 算到模块头上。对照：**没被撤销**的同一个断连模块仍是 502 |
