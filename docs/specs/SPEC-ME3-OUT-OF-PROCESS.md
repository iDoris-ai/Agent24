...
| 「撤 generation 早于杀进程」怎么保证 | `terminate_group` **必须消费一个 `KillPermit`**，而它只能由 `Generation::revoke` 产出（不能手造、不能 `Clone`、没有 `Default`） | 靠「每个调用方都记得顺序」成立的安全性质，在第一个忘了的调用方那里失效。崩溃（死掉的 leader 留下的 helper 可能还攥着回调连接）、启动超时、disable 三条路一律经过它 —— **任何豁免都是一个构造器**。**但它保证的比这句话听上去窄**：许可不绑定到某个进程或某一代 —— 任何一代的许可都能杀任何进程组；`Launched::child` 是 `std::process::Child`，`.kill()` 不要许可。类型只保证「调用 `terminate_group` 之前**某一次** `revoke` 发生过」。把许可和它那一代的进程绑在一起、并让 child 只被一个类型持有，是尚未写出的 `Supervisor` 的事（FU-46） |
