...
**不要试图把来源头列全。** `x-forwarded-*` 能整族剥掉是因为它们共享前缀；而 `X-Real-IP` / `True-Client-IP` / `CF-Connecting-IP` / `X-Client-IP` 不共享任何前缀，某些库（npm 的 `request-ip` 就是）会挨个去读。往名单里继续加名字**没有尽头**，而这正是本文件 SPEC-ME3 §2.1 末尾那条「弱处」说的那种情况——不是新问题，是同一个问题的又一个面。

**收口不在内核穷举，在这一条**：模块不应当信任**任何**来源头。所以本项真正要交付的不是「把剩下那些也剥掉」，而是给出一个模块**可以**信任的东西，并说清它是什么、不是什么。
