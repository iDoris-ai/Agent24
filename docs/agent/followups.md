...
- [ ] FU-16 · **A · F5 阻塞级** · src=reference-notes/proma.md §1 · 2026-08-28 · **两个渠道桥都不处理机器睡眠/唤醒**（已验证：`grep -rn powerMonitor|suspend|unlock-screen apps/desktop/src packages/*-bridge/src` 零匹配）。macOS 睡眠后 TCP 长连接常处于「不报错但永不收消息」的僵尸态 → 错误路径不触发、重连不启动、launchd 不会救。**F5 是「Mac mini 连跑 7 天」，必然多次休眠。这是最可能让泡测在第二天早上静默失效的一条。** 修法：桥进程加定时健康检查 + 主动探活重连（不必依赖 Electron powerMonitor，桥是独立 Node 进程）。参考 Proma 的两段恢复延迟 [1.5s, 10s]
- [ ] FU-17 · **A · F5 阻塞级** · src=reference-notes/proma.md §5 · 2026-08-28 · `packages/wechat-bridge/src/main.ts:36` 的 `wechat-sessions.json` 用裸 `writeFileSync`，无原子写。断电/强制关机会截断该文件 → 所有微信用户的会话映射丢失。修法：write-to-temp → rename（POSIX 原子）+ .bak 回退。**五行代码**
