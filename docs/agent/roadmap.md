...
[`tasks.md`](tasks.md) 顶部的权威状态段。

**排期假设已被实际执行顺序推翻**：本文档 2026-08-23 写下时判断「本轮只做 M1（记忆成为产品）」，
把 **M2（M-E 收口余项）标成「本轮不做，登记在案」**。实际发生的事情正好相反 ——
从 2026-08 下旬起到今天，**M-E / ME-3（进程外领域 OS）是唯一在推进的工作，占据了全部开发精力**：
`tasks.md` 里 T7 / T8 / T8.5a / T8.5b / T8.5c-P / T8.5c-W-mount / T8.5c-W-wire 一路做到 2026-09-19
（PR #196–#221）；而本文档称为「本轮唯一该做的事」的 M1，**F1.1 的第一个 task 至今没有开工记录**。

**用户裁决（2026-09-19）**：路线按 `tasks.md` 既定顺序走完 ME-3 ——
**T8.5c-W-wire 实现 → T9 → T10 → T11 → T12（发布 v0.5.0）**；
**M1 暂不捡回，等 v0.5.0 发布之后再说**。

下面 M1–M6 的内容**保留不改**：它们仍然是未来要做的事，只是**都不是当前在做的事**。
其中 M2 那句「本轮不做」应读作「2026-08 那一轮的判断」，而不是对今天的描述 —— 今天 M-E 就是全部。
...
> ~~**当前聚焦：M1 / F1.1。**~~（2026-08-23 的判断，**已过期** —— 当前聚焦是 ME-3 / v0.5.0，见本文档顶部的现状说明。）
