...
- **T8.5 ME-3d 记忆回调**（`private/*`）——原计划一次性交付，设计文档 [`T8.5-ME3d-memory-private.md`](../design/T8.5-ME3d-memory-private.md)（worktree `Agent24-t8.5`）走了 3 轮 Codex 设计审查仍未收敛（v1 2 Critical/6 High/5 Medium，v2 2 Critical/4 High，v3 仍 2 Critical 未关掉，且 v3 为了关 FU-54 改 `agent24-os-proto` 的 `drain.rs` 请求生命周期信号时，破坏了现有 `progress()`/`revoke()`/`admit_approval_callback()` 依赖 `in_flight` map 成员即"活着"这条不变式——Codex 判定"改动已不是 pure additive，牵动共享基础设施"，建议拆分。2026-09-17 用户拍板采纳，按 Codex 给的三段切法重排（**编号变更，不再用单一 T8.5**）：
  - **T8.5a 请求生命周期信号**（`agent24-os-proto`/`drain.rs`，闭 FU-54）`READY`——正确的"请求已结束"信号必须能让**尚未订阅**的后来者也读到（`watch::Sender::send` 对零订阅者返回 `Err` 且不保留新值，需要 `send_replace`/`send_modify` 或常驻 receiver），且不能通过"让已结束的请求继续留在 `in_flight` map 里"来实现——那会让 `progress()`（drain 是否已空闲）、`revoke()`（撤销上报）、`admit_approval_callback()`（审批 token 有效期）全部读到错误的"仍存活"状态。需要独立的、有界的"已结束请求"记录结构，不与现有活跃请求表混用。
  - **T8.5b 权威事件用量计数 + 迁移/rekey**（`agent24-memory`，为真配额打底）`BLOCKED on T8.5a 完成后独立排期`（不依赖 T8.5a 产出，但按顺序在其后设计）——现有 `EventLog::append`/`append_tx` 两条插入路径 + `rekey_os_partition` 改 `scope_owner` 都不经过计数器，任何一条不接入都会让配额计数器不权威（尤其 rekey 场景：迁移 0014 先按旧 owner 回填用量，`MemoryLease::open` 后续的 legacy partition rekey 才把 `mem_events` 挪到新 owner，用量表却没跟着挪，新 owner 看起来零历史用量）。需要触发器或统一的记账入口覆盖 insert/update/rekey 全部路径。
  - **T8.5c `_a24/memory/private/*` RPC 族本身**——依赖 T8.5a（请求生命周期）+ T8.5b（权威配额）都落地后才能设计分页/取消/配额的最终形态，`BLOCKED on T8.5a, T8.5b`。
...
- **T9 ME-3f 仓外包端到端 —— 验收**（黑盒：不改源码、不重新构建，装一个仓库之外的包，重启后挂载 → 路由代理 → 事件转发全绿）`BLOCKED on T8.5a/T8.5b/T8.5c`（T7、T8 均已 DONE）
