...
|---|---|
| Database is source of truth, AI is not | `agent24-store`:sqlx SQLite,写入走 `BEGIN IMMEDIATE`,先按 `agent24-core` 迁移矩阵校验再落库 |
| AI 输出只是 Proposal,落库必过确定性校验 | `agent24-policy`:fail-closed 审批门 + store 事务校验 |
...
| 实体 | 状态机 | 说明 |
|---|---|---|
| **Direction** 长期方向 | `draft → active → {achieved, abandoned, paused}`;`paused → active` | 月/季度级方向 |
| **Rhythm** 节奏 | `active → adjusted → retired` | 某 Direction 下的目标时间占比(如 Coding 60%) |
| **Week** 周容器 | `planning → active → reviewing → closed` | 一周计划的生命周期 |
| **Task** | `backlog → planned → in_progress → {done, dropped, carried_over}` | carried_over 生成下周新 Task 并留链 |
| **ScheduleBlock** 时间块 | `planned → started → {completed, skipped}` | 计划的执行块,实际由事件对账 |
| **Review** 复盘 | `draft → finalized` | daily / weekly / rhythm 三型 |
| **AttentionBudget** 注意力预算 | (非状态机,物化视图) | 按 Direction 的 planned_ratio vs actual_ratio,纯事件回放算出 |

### 3.2 事件与对账(Sin90 的灵魂)

- `sin90_events` append-only。任何 Sin90 状态变更**必须**产生事件;无事件的状态变更视为 bug。
- `attention_daily` 为物化视图,由事件回放增量更新。
- **SPIKE-00 判定**:`GET /api/v1/sin90/attention?window=week` 必须能纯从事件回放算出「本周 Coding 18h / Business 2h」,不依赖任何对话上下文。

### 3.3 Proposal 门(AI 不写库)

AI(本地脑或 Codex)产出的一切都是 `Sin90Proposal`,经确定性校验后转事务落库并产事件。**复用 `agent24-policy` 的 fail-closed 审批 + `agent24-store` 事务**,不新造机制。
...
1. **`agent24-sin90` 领域 crate**:§3 的实体 / 状态机 / 事件日志 / Proposal 门(复用 store·core·policy)。
...
6. **API 面 + 客户端**:`agent24d` 新增 Sin90 路由(§5)、WS `sin90.*` 事件;`@agent24/api-client` 出 typed client 供壳消费。
...
# 事件流(复用现有 WS)
GET  /api/v1/events                                  # 新增 sin90.* 事件类型
     # sin90.direction.created / task.transitioned / week.closed /
     # proposal.pending / attention.updated / nudge.due ...
```

Reflex(壳内 FSM)不走这些接口;它只在需要落库时(如「用户完成了任务」)发一条 Sin90 事件/命令。
